...
util_wss = require "util_wss"
util_mqtt = require "util_mqtt"
...
sms.setNewSmsCb(function(sender_number, sms_content, m)
    ...
    if util_wss and util_wss.report_contact and config.TRANSPORT == "wss" then
        util_wss.report_contact(sender_number, nil, os.time())
    end
    -- 发送通知
...
sys.taskInit(function()
    ...
    -- ==== 传输层选择：wss / mqtt / none ====
    local mode = (config.TRANSPORT or "none"):lower()
    if mode == "wss" then
        util_wss.init()
        log.info("transport", "WSS enabled")
    elseif mode == "mqtt" then
        util_mqtt.init()
        log.info("transport", "MQTT enabled")
    else
        log.info("transport", "DISABLED (none)")
    end
    ...
    sys.timerLoopStart(function()
        ...
        -- 在重启前, 通知 WSS 我们要下线了
        if mode == "wss" then
            util_wss.notify_shutdown()
        elseif mode == "mqtt" and util_mqtt and util_mqtt.notify_shutdown then
            util_mqtt.notify_shutdown()
        end
        -- 延迟 2 秒钟重启, 确保 "offline" 消息有时间发出
        sys.timerStart(rtos.restart, 2000)
        -- rtos.restart()  -- 重启设备
